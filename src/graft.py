...
import models.dit_backup

# changed grafting code for running default fintuned layer selection grafting
...
def graft_dit(dit_model_name, dit_ckpt_path, image_size,
    ...
    if dit_model_name == "DiT-XL/2":
        ...
        #print("Missing keys when loading base pretrained weights:", missing)
        #print("Unexpected keys when loading base pretrained weights:", unexpected)
    ...
    for index in graft_indexes:
        ...
        if operator_type == 'mha':
            model.blocks[index].attn_grafting = scion_model
    ...
    if dit_ckpt_path is not None:
        ...
        """
        Change the keys in the state_dict that match the pattern 
        'blocks.{i}.attn.*' —
        if the index {i} is included in graft_indexes, 
        rename them to 'blocks.{i}.attn_grafting.*'.
        """
        new_state_dict = {}
        for key, value in state_dict.items():
            ...
            for key, value in state_dict.items():
                replaced = False
                for idx in graft_indexes:
                    target_prefix = f"blocks.{idx}.attn."
                    if key.startswith(target_prefix):
                        # change key to attn_grafting
                        new_key = key.replace(f"blocks.{idx}.attn.", f"blocks.{idx}.attn_grafting.")
        ...
        missing, unexpected = model.load_state_dict(state_dict, strict=False) # For grafting using heterogenous operators.
        print(f'> Model={dit_model_name}, Image size={image_size}, loaded {ckpt_path} successfully')
        #print("Loaded weights for grafted operators from checkpoint.", new_state_dict.keys())
        #print("Missing keys when loading graft pretrained weights:", missing)
        #print("Unexpected keys when loading graft pretrained weights:", unexpected)

    return model



def graft_dit_finetune(dit_model_name, dit_ckpt_path, image_size,
                    operator_type, operator_name, operator_config_filepath, 
                    graft_indexes, graft_weights, 
                    run_all_unit_tests=True):
    """
    ...
    latent_size = image_size // 8
    #!!!!!!!!!!!!!!!!!!!!!!! use model backup!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
    model = models.dit_backup.DiT_models[dit_model_name](
        input_size=latent_size,
        num_classes=1000
    ).to(device)
    ...
    if dit_ckpt_path is not None:
        ...
        missing, unexpected = model.load_state_dict(state_dict, strict=False) 
        print(f'> Model={dit_model_name}, Image size={image_size}, loaded {ckpt_path} successfully')
        print("Loaded weights for grafted operators from checkpoint.", state_dict.keys())
        print("Missing keys when loading graft pretrained weights:", missing)
        print("Unexpected keys when loading graft pretrained weights:", unexpected)


        # if graft_fusion_ckpt_path is not None:
        #     torch_fusion_state_dict = torch.load(graft_fusion_ckpt_path, map_location='cuda')['ema']
        #     model.load_state_dict(torch_fusion_state_dict, strict=True)
        #     print(f'> Loaded graft fusion weights from {graft_fusion_ckpt_path} successfully')
...
def graft_dit_finetune_inference(dit_model_name, dit_ckpt_path, image_size,
                    operator_type, operator_name, operator_config_filepath, 
                    graft_indexes, graft_weights, 
                    run_all_unit_tests=True):
    """
